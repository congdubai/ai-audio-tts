...
# --- Video History ---
def add_video_history_entry(item_id: str, text: str, speed: float, duration: float, filename: str, file_size: int):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO video_history (id, text, speed, duration, filename, file_size, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (item_id, text, speed, duration, filename, file_size, datetime.now().isoformat()))
        conn.commit()

def get_video_history(limit: int = 50) -> List[Dict[str, Any]]:
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, text, speed, duration, filename, file_size, created_at
            FROM video_history
            ORDER BY created_at DESC
            LIMIT ?
        """, (limit,))
...
def get_video_history_by_id(item_id: str) -> Optional[Dict[str, Any]]:
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
...
def delete_video_history_entry(item_id: str) -> bool:
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM video_history WHERE id = ?", (item_id,))
        conn.commit()
        return cursor.rowcount > 0
