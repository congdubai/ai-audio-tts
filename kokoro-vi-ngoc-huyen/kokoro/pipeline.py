...
VI_NUMBER_TOKEN_RE = re.compile(r'\d+(?:[.,:/-]\d+)*')
VI_CURRENCY_RE = re.compile(r'(\d+(?:[.,]\d+)*)\s*(đ|đồng|vnd|vnđ)\b', re.IGNORECASE)
VI_PERCENT_RE = re.compile(r'(\d+(?:[.,]\d+)*)\s*%')
VI_COMPACT_HOUR_RE = re.compile(r'\b(\d{1,2})\s*h\s*(\d{1,2})\b', re.IGNORECASE)
VI_DIGIT_LETTER_BOUNDARY_RE = re.compile(r'(?<=\d)(?=[A-Za-zÀ-ỹĐđ])')
VI_TEXT_FIX_REPLACEMENTS = [
    ('ẩn ẩc nước', 'ngấn nước'),
    ('cậ đừng', 'cậu đừng'),
]
...
def _vi_number_to_words(value: int) -> str:
    from num2words import num2words

    return num2words(value, lang='vi')


def _vi_digits_to_words(digits: str) -> str:
    return ' '.join(_vi_number_to_words(int(ch)) for ch in digits)


def _vi_is_grouped_integer(token: str) -> bool:
...
def _normalize_vi_number_token(token: str) -> str:
    if _vi_is_grouped_integer(token):
        return _vi_number_to_words(int(re.sub(r'[.,]', '', token)))
    if re.fullmatch(r'\d+[.,]\d+', token):
        head, tail = re.split(r'[.,]', token, maxsplit=1)
        return f'{_vi_number_to_words(int(head))} phẩy {_vi_digits_to_words(tail)}'
    if '/' in token:
        return ' trên '.join(_normalize_vi_number_token(part) for part in token.split('/'))
    if ':' in token:
        head, tail = token.split(':', maxsplit=1)
        return f'{_normalize_vi_number_token(head)} giờ {_normalize_vi_number_token(tail)}'
    if '-' in token:
        return ' '.join(_normalize_vi_number_token(part) for part in token.split('-'))
    return _vi_number_to_words(int(token))


def _normalize_vi_text(text: str) -> str:
    text = text.replace('’', "'").replace('‘', "'")
    for old, new in VI_TEXT_FIX_REPLACEMENTS:
        text = text.replace(old, new)
    text = VI_COMPACT_HOUR_RE.sub(
        lambda m: f'{_normalize_vi_number_token(m.group(1))} giờ {_normalize_vi_number_token(m.group(2))}',
        text,
    )
    text = VI_DIGIT_LETTER_BOUNDARY_RE.sub(' ', text)
    text = VI_CURRENCY_RE.sub(lambda m: f'{_normalize_vi_number_token(m.group(1))} đồng', text)
    text = VI_PERCENT_RE.sub(lambda m: f'{_normalize_vi_number_token(m.group(1))} phần trăm', text)
    text = VI_NUMBER_TOKEN_RE.sub(lambda m: _normalize_vi_number_token(m.group(0)), text)
    ...
    def __init__(
        ...
        if lang_code in 'ab':
            ...
            try:
                ...
                logger.error("You need to `pip install vig2p==0.1.0` to use lang_code='v'")
                raise
        else:
